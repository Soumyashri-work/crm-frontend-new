...
async function _syncUser(keycloak, mounted, setUser, setLoading) {
  ...
  const baseUser = {
    ...
    tenant_name: null,
    agent_id:    null,   // filled below for agents
  };
  ...
  // 2. Fetch tenant name — admin and agent only
  ...
  // 3. Fetch agent_id by email — agents only
  if (primaryRole === 'agent' && baseUser.email) {
    try {
      const agentRes = await fetch(
        `${base}/agents/by-email?email=${encodeURIComponent(baseUser.email)}`,
        { headers }
      );
      if (agentRes.ok) {
        const json = await agentRes.json();
        // Backend wraps response in { data: { id, email, ... } } via success()
        const agent = json?.data;
        if (mounted && agent?.id) {
          setUser((prev) => ({ ...prev, agent_id: agent.id }));
        }
      } else {
        console.warn('Agent lookup by email failed:', agentRes.status);
      }
    } catch (err) {
      console.warn('Agent ID fetch failed:', err.message);
    }
  }
}
