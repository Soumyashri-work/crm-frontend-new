...
//  *   GET    /integrations/{id}/credentials/status       → metadata, no secrets
//  *   GET    /integrations/{id}/verify                   → on-demand health check
//  *   GET    /integrations/{id}/webhook-url              → webhook URL + instructions
//  *   POST   /integrations/{id}/credentials/rotate       → Infisical key re-encryption (admin)
//  *
//  * Payload contract (POST /integrations/):
//  *   ProvisionCredentialsRequest:
...
//  *     token_expires_at?, created_at, updated_at }
//  *
//  * WebhookUrlResponse (GET /integrations/{id}/webhook-url):
//  *   { integration_id, webhook_uuid, webhook_url, crm_type, instructions }
//  *
//  * 403 error shape (PermissionValidationError):
//  *   { detail: { error: "insufficient_crm_permissions", message: string, failed_checks: string[] } }
//  *
...
//   /** Webhook URL sub-key (separate so URL fetches don't bust credential caches) */
//   webhookUrl: (id) => ['integrations', id, 'webhook-url'],
// };
...
//   /**
//    * GET /integrations/{integrationId}/webhook-url
//    * Fetch the webhook endpoint URL and CRM-specific setup instructions
//    * for an existing integration. Nothing is mutated.
//    *
//    * Response shape (WebhookUrlResponse):
//    *   { integration_id, webhook_uuid, webhook_url, crm_type, instructions }
//    *
//    * @param {string} integrationId
//    * @returns {Promise<WebhookUrlResponse>}
//    */
//   fetchWebhookUrl: async (integrationId) => {
//     try {
//       const res = await api.get(`/integrations/${integrationId}/webhook-url`);
//       return res.data;
//     } catch (err) {
//       throw buildError(err);
//     }
...
//   if (enable_webhooks) {
//     const crmWebhookModel = entry._webhookModel;
...
//  *
//  * Maps to: GET /integrations/{id}/webhook-url
//  *
//  * Callers should pass `enabled: false` (or omit the integrationId) until the
//  * integration exists and webhook data is actually needed — e.g.:
//  *
//  *   // Only fetch when viewing a locked existing integration:
//  *   const { data } = useGetWebhookUrl(integrationId, {
//  *     enabled: !!integrationId && isLocked,
//  *   });
//  *
//  *   // Only fetch after a successful provision where webhooks were enabled:
//  *   const { data } = useGetWebhookUrl(successData?.integration_id, {
//  *     enabled: !!successData && webhooksWereEnabled,
//  *   });
//  *
//  * Response shape (WebhookUrlResponse):
//  *   { integration_id, webhook_uuid, webhook_url, crm_type, instructions }
//  *
//  * staleTime is set to 5 minutes — the webhook URL is stable for the lifetime
//  * of an integration and only changes on deprovision + re-provision.
//  *
...
//   return useQuery({
//     queryKey: integrationKeys.webhookUrl(integrationId),
//     queryFn:  () => integrationApi.fetchWebhookUrl(integrationId),
//     enabled:  !!integrationId,
//     staleTime: 5 * 60 * 1000,
//     retry: 1,
//     ...options,
...
//  * Update (replace) credentials for an existing integration.
//  *
//  * Maps to: PATCH /integrations/{id}/credentials
//  *
//  * @param {object} [options]
//  */
// export function useUpdateCredentials(options = {}) {
...
export const integrationKeys = {
  ...
  /**
   * Full detail by (tenantId, sourceSystemId) — resolves integration_id,
   * decrypted credentials, and webhook secrets in one call.
   * Used on PageConfigureCrm mount to hydrate the form for existing integrations.
   */
  integrationDetail: (tenantId, sourceSystemId) =>
    ['integrations', 'detail-by-source', tenantId, sourceSystemId],
};
...
export const integrationApi = {
  ...
  /**
   * GET /tenant-source-systems/integration-detail
   * Resolve the integration from (tenant_id, source_system_id), decrypt its
   * credentials, and return the full detail JSON.
   *
   * Response shape (TenantIntegrationDetailResponse):
   *   {
   *     integration_id, tenant_id, source_system_id,
   *     crm_type, auth_type, key_version,
   *     base_url, webhook_uuid, is_active,
   *     credentials, webhook_secrets,
   *     token_expires_at, created_at, updated_at
   *   }
   *
   * Used by PageConfigureCrm to hydrate the form with the real integration_id
   * (and base_url / auth_type) when the admin opens a CRM that was provisioned
   * in a previous session — so the Disconnect / Reset buttons are always shown,
   * the webhook URL is always fetched, and form fields are pre-filled correctly.
   *
   * @param {string} tenantId       — UUID of the tenant (from auth context)
   * @param {number} sourceSystemId — integer source system ID
   * @returns {Promise<TenantIntegrationDetailResponse>}
   */
  fetchIntegrationDetail: async (tenantId, sourceSystemId) => {
    try {
      const res = await api.get('/tenant-source-systems/integration-detail', {
        params: { tenant_id: tenantId, source_system_id: sourceSystemId },
      });
      return res.data;
    } catch (err) {
      throw buildError(err);
...
/**
 * Fetch the full integration detail for a (tenant_id, source_system_id) pair.
 *
 * Maps to: GET /tenant-source-systems/integration-detail
 *
 * This is the key hook for solving the "form blanks out after refresh" problem.
 * When an admin opens a CRM card that was integrated in a prior session, this
 * query fires immediately (enabled as soon as tenantId + sourceSystemId are
 * known) and returns:
 *   • integration_id  — so Disconnect / Reset / webhook-url queries work
 *   • base_url        — pre-fills the Base URL field
 *   • auth_type       — restores the correct auth method radio selection
 *   • crm_type        — confirms the CRM
 *   • is_active       — gate for locking the form
 *   • webhook_uuid    — used to derive webhook state (has_webhooks)
 *
 * Credentials (token, password, etc.) are deliberately NOT pre-filled into
 * form inputs for security — the form stays locked (read-only) showing only
 * the non-secret fields, and the admin must click Reset to enter new ones.
 *
 * On 404 (no integration exists for this pair): returns null gracefully.
 * On any other error: surfaces via `error` as usual.
 *
 * @param {string|null|undefined} tenantId       — UUID from auth context
 * @param {number|null|undefined} sourceSystemId — integer source system ID
 * @param {object}                [options]       — merged into useQuery options
 */
export function useIntegrationDetail(tenantId, sourceSystemId, options = {}) {
  return useQuery({
    queryKey: integrationKeys.integrationDetail(tenantId, sourceSystemId),
    queryFn: async () => {
      try {
        return await integrationApi.fetchIntegrationDetail(tenantId, sourceSystemId);
      } catch (err) {
        // 404 means no integration exists yet — return null instead of throwing
        // so callers can distinguish "not integrated" from a real API error.
        if (err?.status === 404) return null;
        throw err;
      }
    },
    enabled: !!tenantId && sourceSystemId != null,
    staleTime: 60 * 1000, // 1 min — stable between sessions; invalidated on provision/deprovision
    retry: 1,
    ...options,
  });
}
...
export function useProvisionIntegration(options = {}) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: integrationKeys.all() });
      // Bust detail-by-source so the next open of this CRM re-fetches the full detail
      queryClient.invalidateQueries({ queryKey: ['integrations', 'detail-by-source'] });
...
export function useDeprovisionIntegration(options = {}) {
  ...
  return useMutation({
    ...
    onSuccess: (_data, { integrationId }) => {
      queryClient.removeQueries({ queryKey: integrationKeys.detail(integrationId) });
      // Also bust all detail-by-source queries so the form re-fetches on next open
      queryClient.invalidateQueries({ queryKey: ['integrations', 'detail-by-source'] });
      queryClient.invalidateQueries({ queryKey: integrationKeys.all() });
      options.onSuccess?.();
    },
